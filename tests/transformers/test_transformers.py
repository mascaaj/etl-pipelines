...
class TestXetraETLMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.s3_bucket_trg = S3BucketConnector(bucket=self.s3_bucket_name_trg,
                                               secret_key=self.s3_secret_key,
                                               access_key=self.s3_access_key,
                                               endpoint_url=self.s3_endpoint_url)
        self.src_bucket = self.s3.Bucket(self.s3_bucket_name_src)
        self.s3_bucket_src = S3BucketConnector(bucket=self.s3_bucket_name_src,
                                               secret_key=self.s3_secret_key,
                                               access_key=self.s3_access_key,
                                               endpoint_url=self.s3_endpoint_url)
    ...
    def test_transform_report1_emptydf(self):
        ...
        # Expected results
        self.fixture_setup()
        log_exp = 'Dataframe is empty, no transformation will be applied'
        # Test init
        extract_date = '2021-04-17'
    ...
    def test_transform_report1_ok(self):
        ...
        # Expected results
        self.fixture_setup()
        log1_exp = 'Applying transformation to the Xetra source data - report 1'
        log2_exp = 'Transformation complete'
    ...
    def test_load(self):
        ...
        # Expected results
        self.fixture_setup()
        log1_exp = 'Xetra data sucessfully written.'
        log2_exp = 'Meta file has been updated'
        df_exp = self.df_report
        ...
        with patch.object(MetaProcess, "return_date_list",
            ...
            with self.assertLogs() as logm:
                ...
                # Log test after method execution
                self.assertIn(log1_exp, logm.output[0])
                self.assertIn(log2_exp, logm.output[3])
        # Test after method execution
        ...
        df_meta_result = self.s3_bucket_trg.read_s3_to_df(meta_file, 'csv')
        self.assertEqual(list(df_meta_result['source_data']), meta_exp)
        # Cleanup after test
        self.fixture_teardown(trg_file, trg_file)
